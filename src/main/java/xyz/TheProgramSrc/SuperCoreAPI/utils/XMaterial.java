...
public enum XMaterial {
    ...
    BROWN_MUSHROOM_BLOCK("BROWN_MUSHROOM", 0),
    ...
    XMaterial(String material, int... datas) {
    ...
    public ItemStack parseItem() {
        ...
        return isNewVersion() ? new ItemStack(material) : new ItemStack(material, 1, (byte)this.datas[0]);
    }

    public void setMaterialOfItem(ItemStack item) {
        Material material = this.parseMaterial();
        if (isNewVersion()) {
            item.setType(material);
        } else {
            item.setType(material);
            item.setDurability((byte)this.datas[0]);
        }
    }
    ...
    public static boolean isNewVersion() {
        if (newV == 0) {
            ...
            return true;
        } else if (Material.matchMaterial("RED_WOOL") != null) {
            newV = 1;
            return true;
        } else {
            newV = 0;
    ...
    public static XMaterial requestXMaterial(String material, byte data) {
        if (cachedSearch.containsKey(material.toUpperCase() + "," + data)) {
            return cachedSearch.get(material.toUpperCase() + "," + data);
        } else {
            XMaterial[] values = values();

            for (XMaterial xMaterial : values) {
                if ((data == 0 || !xMaterial.isInNewVersion()) && material.toUpperCase().equals(xMaterial.material) && ArrayUtils.contains(xMaterial.datas, data)) {
                    cachedSearch.put(xMaterial.material + "," + data, xMaterial);
                    return xMaterial;
                }
            }
    ...
    public boolean isSameMaterial(ItemStack itemStack) {
        ...
        } else if (itemStack.getType() == this.parseMaterial() && itemStack.getData().getData() == this.datas[0]) {
            return true;
        } else {
            XMaterial xMaterial = fromMaterial(itemStack.getType());
            return this.isDamageable(xMaterial) && this.parseMaterial() == itemStack.getType();
        }
    }

    public static XMaterial fromBlock(Block block) {
        return isNewVersion() ? Arrays.stream(values()).filter((xMaterial) -> xMaterial.name().equals(block.getType().name())).findFirst().orElse(null) : requestXMaterial(block.getType().name(), block.getData());
    }

    public static XMaterial fromItem(ItemStack itemStack) {
        return isNewVersion() ? Arrays.stream(values()).filter((material) -> material.name().equals(itemStack.getType().name())).findFirst().orElse(null) : requestXMaterial(itemStack.getType().name(), itemStack.getData().getData());
    }
    ...
    public static XMaterial fromMaterial(Material material) {
        try {
            ...
            XMaterial[] xMaterials = values();

            for (XMaterial xMaterial : xMaterials) {
                if (xMaterial.material != null && xMaterial.material.equals(material.toString())) {
                    return xMaterial;
                }
            }
    ...
    public static XMaterial fromNameString(String name) {
        XMaterial xMaterial;

        try {
            xMaterial = valueOf(name);
            return xMaterial;
        } catch (Exception ex) {
            String[] data = name.split(":");
            if (data.length == 1) {
                xMaterial = requestXMaterial(name, (byte)0);
            } else {
                xMaterial = requestXMaterial(data[0], (byte)Integer.parseInt(data[1]));
            }
    ...
    public boolean isDamageable(XMaterial xMaterial) {
        String[] names = xMaterial.toString().split("_");
    ...
    public MaterialData parseMaterialData() {
        return isNewVersion() ? new MaterialData(this.parseMaterial()) : new MaterialData(this.parseMaterial(), (byte)this.datas[0]);
    }

    public static XMaterial[] items(boolean isNewVersion) {
        return Arrays.stream(values()).filter((material) -> !material.isInNewVersion() || isNewVersion == material.isInNewVersion()).toArray(XMaterial[]::new);
    }

    public static XMaterial[] itemsSupported() {
        return Arrays.stream(values()).filter(XMaterial::isSupported).toArray(XMaterial[]::new);
    }
    ...
    public String getHumanName() {
        return Utils.getEnumName(this);
    }

    public static XMaterial[] scope(String name) {
        return Arrays.stream(values()).filter((material) -> material.name().toLowerCase().contains(name.toLowerCase())).toArray(XMaterial[]::new);
    }

    public XMaterial orElse(XMaterial other) {
        if (this == STONE) {
            return this;
        } else {
            return this.parseMaterial() != Material.STONE ? this : other;
        }
    }
}
