...
package xyz.theprogramsrc.supercoreapi.spigot.utils.xseries;
...
public enum XMaterial {
    ...
    public ItemStack parseItem() {
        ...
        return isNewVersion() ? new ItemStack(mat) : new ItemStack(mat, 1, (byte)this.datas[0]);
    }

    public void setMaterialOfItem(ItemStack is) {
        Material mat = this.parseMaterial();
        if (isNewVersion()) {
            is.setType(mat);
        } else {
            is.setType(mat);
            is.setDurability((byte)this.datas[0]);
        }
    }
    ...
    public static XMaterial requestXMaterial(String name, byte data) {
        if (cachedSearch.containsKey(name.toUpperCase() + "," + data)) {
            ...
            for (XMaterial mat : values()) {
                if ((data == 0 || !mat.isInNewVersion()) && name.toUpperCase().equals(mat.m) && ArrayUtils.contains(mat.datas, data)) {
    ...
    public boolean isSameMaterial(ItemStack comp) {
        ...
        } else if (comp.getType() == this.parseMaterial() && Utils.requireNonNull(comp.getData()).getData() == this.datas[0]) {
            return true;
        } else {
            XMaterial xmat = fromMaterial(comp.getType());
            return this.isDamageable(Utils.requireNonNull(xmat)) && this.parseMaterial() == comp.getType();
        }
    }

    public static XMaterial fromBlock(Block block) {
        return isNewVersion() ? Arrays.stream(values()).filter((m) -> m.name().equals(block.getType().name())).findFirst().orElse(null) : requestXMaterial(block.getType().name(), block.getData());
    }

    public static XMaterial fromItem(ItemStack itemStack) {
        return isNewVersion() ? Arrays.stream(values()).filter((m) -> m.name().equals(itemStack.getType().name())).findFirst().orElse(null) : requestXMaterial(itemStack.getType().name(), itemStack.getData().getData());
    }
    ...
    public static XMaterial fromMaterial(Material mat) {
        try {
            ...
            for (XMaterial xmat : values()) {
                if (xmat.m != null && xmat.m.equals(mat.toString())) {
                    return xmat;
                }
            }
    ...
    public static XMaterial fromNameString(String key) {
        XMaterial xmat;
    ...
    public boolean isDamageable(XMaterial type) {
        ...
        String var0 = split[length - 1];
        byte var1 = -1;
        switch(var0.hashCode()) {
            case -1850133582:
                if (var0.equals("SHEARS")) {
                    var1 = 13;
                }
                break;
            case -1849815901:
                if (var0.equals("SHOVEL")) {
                    var1 = 7;
                }
                break;
            case -1776664470:
                if (var0.equals("LEGGINGS")) {
                    var1 = 2;
                }
                break;
            case -829199152:
                if (var0.equals("TURTLE_HELMET")) {
                    var1 = 10;
                }
                break;
            case -342000110:
                if (var0.equals("TRIDENT")) {
                    var1 = 11;
                }
                break;
            case 65262:
            ...
            case 71710:
                ...
                break;
            case 63384481:
                if (var0.equals("BOOTS")) {
                    var1 = 3;
                }
                break;
            case 79322589:
                if (var0.equals("SWORD")) {
                    var1 = 4;
                }
                break;
            case 139953965:
                if (var0.equals("PICKAXE")) {
                    var1 = 6;
                }
                break;
            case 1456344605:
                if (var0.equals("HORSE_ARMOR")) {
                    var1 = 12;
                }
                break;
            case 1555044533:
                if (var0.equals("CHESTPLATE")) {
                    var1 = 1;
                }
                break;
            case 2048333745:
                if (var0.equals("ELYTRA")) {
                    var1 = 9;
                }
                break;
            case 2127362157:
                if (var0.equals("HELMET")) {
                    var1 = 0;
                }
        }
    ...
    public boolean isSupported() {
        ...
        if (mat != null) {
            ...
            if (this.m != null) {
                Material replacement = Material.matchMaterial(this.m);
    ...
    public static XMaterial[] items(boolean allowNewItems) {
        return Arrays.stream(values()).filter((v) -> !v.isInNewVersion() || allowNewItems == v.isInNewVersion()).toArray(XMaterial[]::new);
    }

    public static XMaterial[] itemsSupported() {
        return Arrays.stream(values()).filter((v) -> v.isSupported()).toArray(XMaterial[]::new);
    }
    ...
    public static XMaterial[] scope(String key) {
        return Arrays.stream(values()).filter((v) -> v.name().toLowerCase().contains(key.toLowerCase())).toArray(XMaterial[]::new);
    }

    public XMaterial orElse(XMaterial alternative) {
        if (this == STONE) {
            return this;
        } else {
            return this.parseMaterial() != Material.STONE ? this : alternative;
        }
    }
}