...
public enum XMaterial {
    ...
    */
    ...
    /**
     * Gets another material if the selected one is not available
     *
     * @param alternative the alternative material
     * @return the alternative material only if the current one is not available.
     */
    ...
    /**
     * Gets a Human-Readable string (Replaces the underscores for spaces)
    ...
    /**
     * Search through the XMaterials for the given key
     * 
     * @param key the key to search for
     * @return the xmaterials found in the search
     */
    public static XMaterial[] scope(String key) {
        return NAMES.entrySet().stream().filter(entry-> entry.getKey().toLowerCase().contains(key.toLowerCase())).map(Map.Entry::getValue).toArray(XMaterial[]::new);
    }

    /**
     * Gets all materials supported in the current version.
     *
     * @return all supported materials in the current version.
     */
    public static XMaterial[] itemsSupported(){
        return NAMES.entrySet().stream().filter(e-> e.getValue().isSupported()).map(Map.Entry::getValue).toArray(XMaterial[]::new);
    }
}